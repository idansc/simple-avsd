...
class Atten(nn.Module):
    def __init__(self, util_e, high_order_utils, prior_flag=False,
                 sizes=[], size_flag=False, pairwise_flag=True, unary_flag=True, self_flag=True):
        ...
        for idx, e_dim in enumerate(util_e):
            self.un_models[idx] = Unary(e_dim)
            if self.size_flag and sizes[idx] != 49:
        ...
        if pairwise_flag:
            ...
            for idx, num_utils, connected_utils in high_order_utils:
                ...
                self.num_of_potentials[idx] += 1 #connected to himself
                self.num_of_potentials[idx] -= ((self.n_utils - 1) - len(connected_utils))




        for idx in range(self.n_utils):
            self.reduce_potentials[idx] = nn.Conv1d(self.num_of_potentials[idx], 1, 1, bias=False)
    ...
    def forward(self, utils, priors=None):
            ...
            if self.size_flag:
                for i, num_utils, _ in self.high_order_utils:
                    if str(i) not in self.spatial_pool.keys():
                        ...
                        utils[i][1] = self.spatial_pool[str(i)](high_util).transpose(1, 2)
            ...
            for i, num_utils, connected_list in self.high_order_utils:
                # i.e. Sharing-Weights utility
                high_util = utils[i][1]
                high_util = high_util.view(num_utils * b_size, high_util.size(2), high_util.size(3))
                util_poten[i] = []
                if self.unary_flag:
                    util_poten[i].append(self.un_models[i](high_util))
                ...
                if self.pairwise_flag:
                    for j in connected_list:
                        other_util = utils[j]
                        expanded_util = other_util.unsqueeze(0).expand(num_utils,
                                                             b_size,
                                                             other_util.size(1),
                                                             other_util.size(2))
                        expanded_util = expanded_util.contiguous().view(num_utils * b_size,
                                                                        other_util.size(1),
                                                                        other_util.size(2))
                        if i < j:
                        ...
                        util_poten.setdefault(j, []).extend([u.squeeze(0) for u in torch.split(poten_ji.view(num_utils, b_size, -1), 1, 0)])
            ...
            for i in range(self.n_utils):
                if i in self.high_order_set:
                    high_util = utils[i][1]
                    high_util = high_util.view(num_utils * b_size,
                                               high_util.size(2), high_util.size(3))
                if self.prior_flag:
                    prior = priors[i] \
                        if priors[i] is not None \
                        else torch.zeros_like(util_poten[i][0]).cuda()

                    util_poten[i].append(prior)

                util_poten[i] = torch.stack(util_poten[i], dim=1)
                util_poten[i] = self.reduce_potentials[i](util_poten[i]).squeeze(1)
                util_poten[i] = F.softmax(util_poten[i], dim=1).unsqueeze(2)
                if i in self.high_order_set:
                    spatial_attention.append(torch.bmm(high_util.transpose(1, 2), util_poten[i])
                                             .view(num_utils, b_size, -1))
                else:
                    attention.append(torch.bmm(utils[i].transpose(1, 2), util_poten[i]).squeeze(2))

            return attention, spatial_attention
